    ...
    <div className="w-full max-h-[600px]">
      ...
      {bannerData?.map((item, index)=>(
        <div key={index} className="relative">
          <img src={item?.image} alt="bannerImage" className="h-[450px] lg:h-[450px] w-full object-cover"/>
          <div className="absolute top-0 left-0 w-full h-full bg-black/20">
            <Container className="flex flex-col justify-center gap-2 md:gap-3 h-full">
              ...
              <p className="font-medium text-sm md:text-base">
                From <span className="md:text-xl font-bold text-blue-700 md:ml-2">${item?.from}</span></p>
              <button onClick={()=>navigate("/shop")} className="w-24 md:w-44 py-2 md:py-0 md:h-12 bg-black/80 text-white rounded-md text-xs 
              md:text-sm uppercase font-semibold hover:bg-black hoverEffect">
                Shop Now</button>
            </Container>
          </div>
        </div>
